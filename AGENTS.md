...
| [`logout-sddm-fix.md`](file:///home/mathieu/docs/logout-sddm-fix.md) | Fix pour le bouton logout Caelestia qui ne relançait pas SDDM. Surcharge de `session.commands.logout` avec `hyprctl dispatch exit`. | `~/.config/caelestia/shell.json`, `hyprctl dispatch exit`, `SessionManager`, logind, SDDM |
